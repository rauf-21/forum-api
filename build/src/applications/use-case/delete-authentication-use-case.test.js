...
const delete_authentication_use_case_1 = require("./delete-authentication-use-case");
describe("DeleteAuthenticationUseCase", () => {
    it("should orchestrate the delete authentication action correctly", async () => {
        ...
        const deleteAuthenticationUseCase = new delete_authentication_use_case_1.DeleteAuthenticationUseCase({
    ...
    it("should throw an error if the payload does not contain a refresh token", async () => {
        const deleteAuthenticationUseCase = new delete_authentication_use_case_1.DeleteAuthenticationUseCase({});
        await expect(deleteAuthenticationUseCase.execute({})).rejects.toThrow(delete_authentication_use_case_error_1.DELETE_AUTHENTICATION_USE_CASE_ERROR.MISSING_REFRESH_TOKEN);
    });
    it("should throw an error if the refresh token is not a string", async () => {
        ...
        const deleteAuthenticationUseCase = new delete_authentication_use_case_1.DeleteAuthenticationUseCase({});
        await expect(deleteAuthenticationUseCase.execute(useCasePayload)).rejects.toThrow(delete_authentication_use_case_error_1.DELETE_AUTHENTICATION_USE_CASE_ERROR.INVALID_REFRESH_TOKEN_DATA_TYPE);
    });
});
